...
def main():
    ...
    parser.add_argument('--save_weights', type=bool, default=False, help='if you need to save weights')parser.add_argument('--basemodel', type=str, default="LeNet_pp", help='basemodel architecture', choices = ['LeNet_pp', 'MiniResnet18'])
    ...
    angleLinear = AngleLinear(in_features=args.visual_dimension, out_features=cnfs["data"]["num_classes"],
    ...
    optimizer = optim.SGD([
                ...
                lr=loss_cnf["optimizer"]["lr"], 
                momentum=loss_cnf["optimizer"]["momentum"])
    scheduler = optim.lr_scheduler.StepLR(optimizer,
                                step_size=loss_cnf["optimizer"]["step_size"],
                                gamma=loss_cnf["optimizer"]["gamma"])
    # make dirs
    ...
    def visualization(features, labels, w, title):
        ...
        plt.legend(['error'] + ['num_'+str(i) for i in range(10)], loc = 'upper right')
        for i in range(10):
    ...
    def test_epoch(epoch_id):
        ...
        if args.save_weights and best_acc < acc:
            print('Saving..')
            state = {
        ...
        # plot and save feature distribution
        title = '{}_test_epoch{:02}'.format(args.lossname, epoch_id)
        w = angleLinear.weight.cpu().detach().numpy()
        plot_features = np.concatenate(plot_features)
        plot_labels = np.concatenate(plot_labels)
        # visualization(plot_features, plot_labels, w, title)
